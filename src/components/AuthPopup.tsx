...
type Step = "welcome" | "phone" | "otp" | "details" | "email" | "email-verify";
...
export default function AuthPopup({ isOpen, onClose, redirectTo, initialStep }: AuthPopupProps) {
  ...
  const [verifySource, setVerifySource] = useState<"email" | "details">("email");
  ...
  const handleCompleteDetails = async (e: React.FormEvent) => {
    ...
    try {
      if (isGoogleCompletion) {
        ...
        setEmailOtpSent(true);
        setEmailOtpExpiresAt(otpData.expiresAt);
        setVerifySource("details");
    ...
    >
      ...
      >
        ...
        {step === "details" && (
          <div>
            ...
            <form onSubmit={handleCompleteDetails} className="space-y-4">
              ...
              >
                {loading 
                  ? (isGoogleCompletion ? "Saving..." : "Processing...") 
                  : (isGoogleCompletion ? "Save & Continue" : "Verify email to continue")
                }
              </button>
            </form>
          </div>
        ...
        {step === "email-verify" && (
          <div>
            <button
              onClick={() => { setStep(verifySource); setError(""); setOtpError(""); setEmailOtp(""); }}
              className="mb-6 flex items-center gap-1 text-sm text-text-secondary hover:text-text-primary transition-colors font-semibold"
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <polyline points="15 18 9 12 15 6" />
            ...
            <h2 className="text-2xl font-bold text-brand-dark mb-1">Verify & Continue</h2>
            <p className="text-sm text-text-muted mb-6">
            ...
            <form onSubmit={handleEmailVerifySubmit} className="space-y-4">
              <div>
                <label className="mb-1.5 block text-sm font-semibold text-brand-dark">Verification Code</label>
                <input
                  type="text"
                  value={emailOtp}
                  onChange={(e) => setEmailOtp(e.target.value.replace(/\D/g, "").slice(0, 6))}
                  className="w-full rounded-[12px] border border-border-default px-4 py-3 text-base outline-none transition-colors focus:border-green-500 focus:ring-1 focus:ring-green-500/20 text-text-primary bg-surface text-center tracking-[6px] font-mono"
                  placeholder="000000"
                  maxLength={6}
                  autoFocus
                />
